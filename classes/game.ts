...
export class Game {
  ...
  constructor(width: number, height: number) {
    ...
    button.addEventListener("click", () => {
      this.toggleGravity();
      button.innerHTML = this.gravityOn ? "Gravity On" : "Gravity Off";
    });
    ...
    modes.addEventListener("click", () => {
      ...
      modes.innerHTML = this.editMode ? "Game Mode" : "Edit Mode";
    });
  ...
  saveLevel() {
    ...
    localStorage.setItem(
      (<HTMLInputElement>document.getElementById("saveInput")).value,
      JSON.stringify(this)
    );
  }
  ...
  async loadLevel() {
    ...
    for (let i = 0; i < loaded.springs.length; i++) {
      ...
      this.springs.push(
        new Spring(
          this,
          this.masses[loaded.springs[i].a.index],
          this.masses[loaded.springs[i].b.index]
        )
      );
    }
  }
  ...
  cycle() {
    ...
    if (this.selectedSpring) {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(
        this.selectedSpring?.a.position.x,
        this.selectedSpring?.a.position.y
      );
      this.ctx.lineTo(
        this.selectedSpring?.b.position.x,
        this.selectedSpring?.b.position.y
      );
      this.ctx.stroke();
    }
